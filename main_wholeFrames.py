    ...
    for test_name in os.listdir(test_folder):
        ...
        if os.path.isdir(current_folder):
            ...
            for sub_test in range(number_test):
                ...
                print(" ********************MaskWPCA_wholeFrame********************")
                timecounter = datetime.now()
                A1_star5 = LWPCA_wholeFrame(np.copy(A_N3_source_added), np.copy(A1zero), db=True)
                # stop
                ...
                # print(" ********************LWPCA_wholeFrame********************")
                # timecounter = datetime.now()
                # A1_star6 = LWPCA_wholeFrame(np.copy(A_N3_source_added), np.copy(A1zero))
                # # stop
                # value = np.around(calculate_mae_matrix(
                ...
                # timecounter = datetime.now()
                # A1_star7 = WPCA_wholeFrame(np.copy(A_N3_source_added), np.copy(A1zero))
                # # stop
                # value = np.around(calculate_mae_matrix(
                ...
                print("********************Li********************")
                timecounter = datetime.now()
                A1_star8 = WPCA_wholeFrame(np.copy(A_N3_source_added), np.copy(A1zero), db=True)
                value = np.around(calculate_mae_matrix(
...
if __name__ == '__main__':
    ...
    f = open(savingLocation + 'resultHDM_wholeFrame.txt', "w")
